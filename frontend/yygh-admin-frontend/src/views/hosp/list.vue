    ...
    <el-table-column label="操作" width="230" align="center">
      <template slot-scope="scope">
        ...
        <router-link :to="'/hospSet/hospital/schedule/'+scope.row.hoscode">
          <el-button type="primary" size="mini" style="margin-left: 10px">排班</el-button>
        </router-link>
        <el-button v-if="scope.row.status==1" style="margin-left: 10px"
                   @click.native.prevent="updateStatus(scope.row.id, 0)"
                   type="danger"
                   size="mini">
          下线
        </el-button>
        <el-button v-else="scope.row.status==0" style="margin-left: 10px"
                   @click.native.prevent="updateStatus(scope.row.id, 1)"
                   type="primary"
                   size="mini">
          上线
        </el-button>
...
export default {
  ...
  methods: {
    fetchData(page = 1){
      this.page = page
      hosp.getHospList(page, this.limit, this.searchObj)
      .then(res => {
    ...
    provinceChanged(){
      ...
      hosp.getCity(this.searchObj.provinceCode)
      .then(res => {
